...
public class ParsingExample {
	...
	/**
	 * Example of how to use Tika's parseToString method to parse the content of
	 * a file, and return any text found.
	 *
	...
	/**
	 * Example of how to use Tika to parse a file when you do not know its file
	 * type ahead of time.
	 *
	 * AutoDetectParser attempts to discover the file's type automatically, then
	 * call the exact Parser built for that file type.
	 *
	 * The stream to be parsed by the Parser. In this case, we get a file from
	 * the resources folder of this project.
	 *
	 * Handlers are used to get the exact information you want out of the host
	 * of information gathered by Parsers. The body content handler,
	 * intuitively, extracts everything that would go between HTML body tags.
	 *
	 * The Metadata object will be filled by the Parser with Metadata discovered
	 * about the file being parsed.
	 *
	 ...
	 * {@link org.apache.tika.parser.ParseContext} that does not contain a
	 * {@link Parser}.
	 *
	 * @return The content of a file.
	 */
	...
	/**
	 * For documents that may contain embedded documents, it might be helpful to
	 * create list of metadata objects, one for the container document and one
	 * for each embedded document. This allows easy access to both the extracted
	 * content and the metadata of each embedded document. Note that many
	 * document formats can contain embedded documents, including traditional
	 * container formats -- zip, tar and others -- but also common office
	 * document formats including: MSWord, MSExcel, MSPowerPoint, RTF, PDF, MSG
	 * and several others.
	 * <p>
	 * The "content" format is determined by the ContentHandlerFactory, and the
	 * content is stored in
	 * {@link org.apache.tika.parser.RecursiveParserWrapper#TIKA_CONTENT}
	 * <p>
	 * The drawback to the RecursiveParserWrapper is that it caches metadata and
	 * contents in memory. This should not be used on files whose contents are
	 * too big to be handled in memory.
	 *
	 ...
	 * {@link org.apache.tika.metadata.serialization.JsonMetadataList}. That
	 * class also includes a deserializer to convert from JSON back to a List
	 * <Metadata>.
	 * <p>
	 * This functionality is also available in tika-app's GUI, and with the -J
	 * option on tika-app's commandline. For tika-server users, there is the
	 * "rmeta" service that will return this format.
	 *
