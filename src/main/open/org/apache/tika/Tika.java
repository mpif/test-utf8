...
/**
 * Facade class for accessing Tika functionality. This class hides much of the
 * underlying complexity of the lower level Tika classes and provides simple
 * methods for many common parsing and type detection operations.
 *
...
public class Tika {
	...
	/**
	 * Creates a Tika facade using the given detector and parser instances, but
	...
	/**
	 * Creates a Tika facade using the given detector, parser, and translator
	 * instances.
	 *
	...
	/**
	 * Creates a Tika facade using the given detector instance, the default
	 * parser configuration, and the default Translator.
	 *
	 * @since Apache Tika 0.8
	 * @param detector
	 *            type detector
	...
	/**
	 * Detects the media type of the given document. The type detection is based
	 * on the content of the given document stream and any given document
	 * metadata. The document stream can be <code>null</code>, in which case
	 * only the given document metadata is used for type detection.
	 * <p>
	 ...
	 *
	 * @param stream
	 *            the document stream, or <code>null</code>
	 * @param metadata
	 *            document metadata
	...
	/**
	 * Detects the media type of the given document. The type detection is based
	 * on the content of the given document stream and the name of the document.
	 * <p>
	 * If the document stream supports the {@link InputStream#markSupported()
	 * mark feature}, then the stream is marked and reset to the original
	 * position before this method returns. Only a limited number of bytes are
	...
	/**
	 * Detects the media type of the given document. The type detection is based
	 * on the content of the given document stream.
	 * <p>
	 * If the document stream supports the {@link InputStream#markSupported()
	 * mark feature}, then the stream is marked and reset to the original
	 * position before this method returns. Only a limited number of bytes are
	...
	/**
	 * Detects the media type of the given document. The type detection is based
	 * on the first few bytes of a document and the document name.
	 * <p>
	...
	/**
	 * Detects the media type of the given document. The type detection is based
	 * on the first few bytes of a document.
	 * <p>
	 * For best results at least a few kilobytes of the document data are
	 * needed. See also the other detect() methods for better alternatives when
	 * you have more than just the document prefix available for type detection.
	...
	/**
	 * Detects the media type of the given file. The type detection is based on
	 * the document content and a potential known file extension.
	 * <p>
	 * Use the {@link #detect(String)} method when you want to detect the type
	 * of the document without actually accessing the file.
	 *
	 * @param file
	 *            the file
	 * @return detected media type
	 ...
	 * detection is based on the document content and a potential known file
	 * extension included in the URL.
	 * <p>
	 * Use the {@link #detect(String)} method when you want to detect the type
	 * of the document without actually accessing the URL.
	 *
	 * @param url
	...
	/**
	 * Detects the media type of a document with the given file name. The type
	 * detection is based on known file name extensions.
	 * <p>
	 * The given name can also be a URL or a full file path. In such cases only
	 * the file name part of the string is used for type detection.
	 *
	 * @param name
	 ...
	 *            The input text language (for example, "hi").
	 * @param targetLanguage
	 *            The desired output language (for example, "fr").
	 * @return The translated text. If translation is unavailable (client keys
	 *         not set), returns the same text back.
	 */
	...
	/**
	 * Translate the given text String to the given language, attempting to
	 ...
	 *            The text to translate.
	 * @param targetLanguage
	 *            The desired output language (for example, "en").
	 * @return The translated text. If translation is unavailable (client keys
	 *         not set), returns the same text back.
	 */
	...
	/**
	 * Translate the given text InputStream to the given language, attempting to
	 * auto-detect the source language. This does not close the stream, so the
	 * caller has the responsibility of closing it.
	 * 
	 * @see org.apache.tika.language.translate.Translator
	 * @param text
	 *            The text to translate.
	...
	/**
	 * Parses the given document and returns the extracted text content. Input
	 * metadata like a file name or a content type hint can be passed in the
	 * given metadata instance. Metadata information extracted from the document
	 * is returned in that same metadata instance.
	 * <p>
	 * The returned reader will be responsible for closing the given stream. The
	 * stream and any associated resources will be closed at or before the time
	 * when the {@link Reader#close()} method is called.
	 *
	 * @param stream
	...
	/**
	 * Parses the resource at the given URL and returns the extracted text
	 * content.
	 *
	 * @param url
	...
	/**
	 * Parses the given document and returns the extracted text content. The
	 * given input stream is closed by this method.
	 * <p>
	 * To avoid unpredictable excess memory use, the returned string contains
	 * only up to {@link #getMaxStringLength()} first characters extracted from
	 * the input document. Use the {@link #setMaxStringLength(int)} method to
	 * adjust this limitation.
	 * <p>
	 * <strong>NOTE:</strong> Unlike most other Tika methods that take an
	 * {@link InputStream}, this method will close the given stream for you as a
	 * convenience. With other methods you are still responsible for closing the
	 * stream or a wrapper instance returned by Tika.
	 *
	 * @param stream
	 *            the document to be parsed
	 * @param metadata
	...
	/**
	 * Parses the given document and returns the extracted text content. The
	 * given input stream is closed by this method. This method lets you control
	 * the maxStringLength per call.
	 * <p>
	 * To avoid unpredictable excess memory use, the returned string contains
	 * only up to maxLength (parameter) first characters extracted from the
	 * input document.
	 * <p>
	 * <strong>NOTE:</strong> Unlike most other Tika methods that take an
	 * {@link InputStream}, this method will close the given stream for you as a
	 ...
	 *            maximum string length, or -1 to disable this limit
	 */
