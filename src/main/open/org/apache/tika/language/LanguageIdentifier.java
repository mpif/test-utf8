...
/**
 * Identifier of the language that best matches a given content profile. The
 * content profile is compared to generic language profiles based on material
 ...
 * @see <a href="http://www.loc.gov/standards/iso639-2/php/code_list.php"> ISO
 *      639 Language Codes</a>
 */
public class LanguageIdentifier {
	...
	/*
	 * Always attempt initializing language profiles when class is loaded first
	 * time
	 */
	static {
		initProfiles();
	...
	private static void addProfile(String language) throws Exception {
		try {
			...
			InputStream stream = LanguageIdentifier.class.getResourceAsStream(language + PROFILE_SUFFIX);
			try {
	 ...
	 * 
	 * @param language
	 *            an ISO 639 code representing language
	 * @param profile
	...
	/**
	 * Tries to judge whether the identification is certain enough to be
	 * trusted. WARNING: Will never return true for small amount of input texts.
	 * 
	 * @return <code>true</code> if the distance is smaller then
	 *         {@value #CERTAINTY_LIMIT}, <code>false</code> otherwise
	 */
	...
	/**
	 * Builds the language profiles. The list of languages are fetched from a
	 * property file named "tika.language.properties" If a file called
	 * "tika.language.override.properties" is found on classpath, this is used
	 * instead The property file contains a key "languages" with values being
	 ...
	 * This overrides the default set of profiles initialized at startup, and
	 * provides an alternative to configuring profiles through property file
	 *
	 * @param profilesMap
