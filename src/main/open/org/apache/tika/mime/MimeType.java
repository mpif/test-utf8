...
public final class MimeType implements Comparable<MimeType>, Serializable {
	...
	/**
	 * Checks that the given string is a valid Internet media type name based on
	 * rules from RFC 2054 section 5.3. For validation purposes the rules can be
	...
	/**
	 * All known file extensions of this type, in order of preference (best
	 * first).
	 */
	 ...
	 * registry. The name is expected to be valid and normalized to lower case.
	 * This constructor should only be called by
	 * {@link MimeTypes#forName(String)} to keep the media type registry up to
	 * date.
	 *
	 * @param type
	 *            normalized media type name
	 */
	...
	static class RootXML implements Serializable {
		...
		boolean matches(String namespaceURI, String localName) {
			...
			if (!isEmpty(this.namespaceURI)) {
				...
				// else if it was empty then check to see if the provided
				// namespaceURI
				// is empty. If it is not, then these two aren't equal and
				// return false
	...
	/**
	 * Returns the preferred file extension of this type, or an empty string if
	 * no extensions are known. Use the {@link #getExtensions()} method to get
	 * the full list of known extensions of this type.
	 *
