...
public class MediaTypeRegistry implements Serializable {
	...
	/**
	 * Registry of known media types, including type aliases. A canonical media
	 * type is handled as an identity mapping, while an alias is stored as a
	 * mapping from the alias to the corresponding canonical type.
	 */
	private final Map<MediaType, MediaType> registry = new HashMap<MediaType, MediaType>();

	/**
	 * Known type inheritance relationships. The mapping is from a media type to
	 * the closest supertype.
	 */
	private final Map<MediaType, MediaType> inheritance = new HashMap<MediaType, MediaType>();

	/**
	 * Returns the set of all known canonical media types. Type aliases are not
	 ...
	 * @since Apache Tika 0.8
	 * @param a
	 *            media type, normalised
	 * @param b
	 *            suspected supertype, normalised
	 * @return <code>true</code> if b is a supertype of a, <code>false</code>
	 *         otherwise
	 */
	...
	/**
	 * Checks whether the given media type equals the given base type or is a
	 * specialization of it. Both types should be already normalised.
	 *
	...
	/**
	 * Parses and normalises the given media type string and checks whether the
	 * result equals the given base type or is a specialization of it. The given
	 * base type should already be normalised.
	 *
	 * @since Apache Tika 1.2
	 * @param a
	 *            media type
	...
	/**
	 * Returns the supertype of the given type. If the media type database has
	 * an explicit inheritance rule for the type, then that is used. Next, if
	 * the given type has any parameters, then the respective base type
	 * (parameter-less) is returned. Otherwise built-in heuristics like text/...
	 * -&gt; text/plain and .../...+xml -&gt; application/xml are used. Finally
	 * application/octet-stream is returned for all types for which no other
	 * supertype is known, and the return value for application/octet-stream is
	 * <code>null</code>.
	 *
	 * @since Apache Tika 0.8
	 * @param type
