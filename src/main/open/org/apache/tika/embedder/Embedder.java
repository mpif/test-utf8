...
public interface Embedder extends Serializable {
	 ...
	 * <p>
	 * The name differs from the precedence of
	 * {@link Parser#getSupportedTypes(ParseContext)} so that parser
	 * implementations may also choose to implement this interface.
	 *
	 * @param context
	 *            parse context
	 ...
	 * <p>
	 * In general implementations should favor preserving the source file's
	 * metadata unless an update to a field is explicitly defined in the
	 * Metadata object. More specifically:
	 * <ul>
	 * <li>Embedder implementations should only attempt to update metadata
	 * fields present in the given Metadata object. Other fields should be left
	 * untouched.</li>
	 ...
	 * corresponding to fields with a null value in the given Metadata object.
	 * </li>
	 * <li>Embedder implementations should set the property corresponding to a
	 ...
	 * particular metadata container falls out of use and/or is superseded by
	 * another (such as IIC vs XMP for IPTC) it is up to the implementation to
	 * decide if and when to cease embedding in the alternate container.</li>
	 * <li>Embedder implementations should attempt to embed as much of the
	 * metadata as accurately as possible. An implementation may choose a strict
	 * approach and throw an exception if a value to be embedded exceeds the
	 * length allowed or may choose to truncate the value.</li>
	 * </ul>
	 * 
	 * @param metadata
	 ...
	 * @param outputStream
	 *            the output stream to write the metadata embedded data to
	 * @param context
	 *            parse context
	 * @throws IOException
