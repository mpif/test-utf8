...
/**
 * Content handler decorator that attempts to prevent denial of service attacks
 * against Tika parsers.
 * <p>
 * Currently this class simply compares the number of output characters to to
 * the number of input bytes and keeps track of the XML nesting levels. An
 * exception gets thrown if the output seems excessive compared to the input
 * document. This is a strong indication of a zip bomb.
 *
...
public class SecureContentHandler extends ContentHandlerDecorator {
	...
	/**
	 * Decorates the given content handler with zip bomb prevention based on the
	 * count of bytes read from the given counting input stream. The resulting
	 * decorator can be passed to a Tika parser along with the given counting
	 * input stream.
	 *
	 * @param handler
	...
	/**
	 * Sets the ratio between output characters and input bytes. If this ratio
	 * is exceeded (after the output threshold has been reached) then an
	 ...
	 * {@link TikaException} if it's caused by this instance detecting a zip
	 * bomb.
	 *
	 * @param e
	 *            SAX exception
	...
	/**
	 * Records the given number of output characters (or more accurately UTF-16
	 * code units). Throws an exception if the recorded number of characters
	 * highly exceeds the number of input bytes read.
	 *
	 * @param length
	 *            number of new output characters produced
	 * @throws SAXException
	...
	private void advance(int length) throws SAXException {
		...
		if (characterCount > threshold && characterCount > byteCount * ratio) {
			throw new SecureSAXException("Suspected zip bomb: " + byteCount + " input bytes produced " + characterCount
					+ " output characters");
		}
	}
