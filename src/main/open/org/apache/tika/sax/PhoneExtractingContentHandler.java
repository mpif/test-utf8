 ...
 *
 * Every time a document is parsed in Tika, the content is split into SAX
 * events. Those SAX events are handled by a ContentHandler. You can think of
 * these events as marking a tag in an HTML file. Once you're finished parsing,
 * you can call handler.toString(), for example, to get the text contents of the
 * file. On the other hand, any of the metadata of the file will be added to the
 * Metadata object passed in during the parse() call. So, the Parser class sends
 * metadata to the Metadata object and content to the ContentHandler.
 *
 ...
 * textual pattern for a phone number. If the extracted content is a phone
 * number, we add it to the metadata under the key "phonenumbers". So, if you
 * used this ContentHandler when you parsed a document, then called
 * metadata.getValues("phonenumbers"), you would get an array of Strings of
