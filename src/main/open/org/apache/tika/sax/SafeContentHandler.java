 ...
 * content handler contain only valid XML characters. All invalid characters are
 * replaced with spaces.
 * <p>
 * The XML standard defines the following Unicode character ranges as valid XML
 * characters:
 * 
 * <pre>
...
public class SafeContentHandler extends ContentHandlerDecorator {
	...
	/**
	 * Internal interface that allows both character and ignorable whitespace
	 * content to be filtered the same way.
	 */
	...
	/**
	 * Output through the
	 * {@link ContentHandler#ignorableWhitespace(char[], int, int)} method of
	...
	/**
	 * Filters and outputs the contents of the given input buffer. Any invalid
	 * characters in the input buffer area handled by sending a replacement (a
	 * space character) to the given output. Any sequences of valid characters
	 * are passed as-is to the given output.
	 * 
	 * @param ch
	 *            input buffer
	 * @param start
	 ...
	 * and should be replaced for output. Subclasses can override this method to
	 * use an alternative definition of which characters should be replaced in
	 * the XML output. The default definition from the XML specification is:
	 * 
	 * <pre>
	...
	/**
	 * Outputs the replacement for an invalid character. Subclasses can override
	 * this method to use a custom replacement.
	 *
	 * @param output
	 ...
	 * 
	 * // Called only from assert private boolean verifyStartElement(String
	 * name) { // TODO: we could strengthen this to do full // XTHML validation,
	 * eg you shouldn't start p inside // another p (but ODF parser, at least,
	 * seems to // violate this): //if (name.equals("p")) { //assert
	 * elements.size() == 0 || !elements.get(elements.size()-1).equals("p"); //}
	 * elements.add(name); return true; }
	 * 
	 * // Called only from assert private boolean verifyEndElement(String name)
	 * { assert elements.size() > 0: "end tag=" + name + " with no startElement"
	 * ; final String currentElement = elements.get(elements.size()-1); assert
	 * currentElement.equals(name): "mismatched elements open=" + currentElement
	 * + " close=" + name; elements.remove(elements.size()-1); return true; }
	 * 
	 * // Called only from assert private boolean verifyEndDocument() { assert
	 * elements.size() == 0; return true; }
	 */
