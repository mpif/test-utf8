...
public class BasicContentHandlerFactory implements ContentHandlerFactory {
	 ...
	 *
	 * @param type
	 *            basic type of handler
	 * @param writeLimit
	 *            max number of characters to store; if < 0, the handler will
	...
	public ContentHandler getNewContentHandler(OutputStream os, String encoding) throws UnsupportedEncodingException {
		...
		if (writeLimit > -1) {
			switch (type) {
			case BODY:
				return new WriteOutContentHandler(new BodyContentHandler(new OutputStreamWriter(os, encoding)),
						writeLimit);
			case TEXT:
