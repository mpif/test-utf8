...
/**
 * Content handler decorator that simplifies the task of producing XHTML events
 * for Tika content parsers.
 */
public class XHTMLContentHandler extends SafeContentHandler {
	...
	/**
	 * The elements that are automatically emitted by lazyStartHead, so skip
	 * them if they get sent to startElement/endElement by mistake.
	 */
	...
	/**
	 * Metadata associated with the document. Used to fill in the &lt;head/&gt;
	 * section.
	 */
	...
	/**
	 * Starts an XHTML document by setting up the namespace mappings when called
	 * for the first time. The standard XHTML prefix is generated lazily when
	 * the first element is started.
	 */
	@Override
	...
	private void lazyEndHead(boolean isFrameset) throws SAXException {
		...
		if (!headEnded) {
			...
			// TIKA-478: Emit all metadata values (other than title). We have to
			// call
			...
			for (String name : metadata.names()) {
				...
				for (String value : metadata.getValues(name)) {
					// Putting null values into attributes causes problems, but
					// is
	...
	/**
	 * Ends the XHTML document by writing the following footer and clearing the
	 * namespace mappings:
	 * 
	 * <pre>
	 *   &lt;/body&gt;
	...
	/**
	 * Ends the given element. Block elements are automatically followed by a
	 * newline character.
	 */
	@Override
	...
	/**
	 * @see <a href="https://issues.apache.org/jira/browse/TIKA-210">TIKA-210
	 *      </a>
	 */
	@Override
	...
	/**
	 * Emits an XHTML element with the given text content. If the given text
	 * value is null or empty, then the element is not written.
	 *
	 * @param name
	 *            XHTML element name
	 * @param value
	 *            element value, possibly <code>null</code>
	 * @throws SAXException
