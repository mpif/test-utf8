...
/**
 * A composite parser based on all the {@link Parser} implementations available
 * through the {@link javax.imageio.spi.ServiceRegistry service provider
 * mechanism}.
 *
...
public class DefaultParser extends CompositeParser {
	...
	/**
	 * Finds all statically loadable parsers and sort the list by name, rather
	 * than discovery order. CompositeParser takes the last parser for any given
	 * media type, so put the Tika parsers first so that non-Tika (user
	 * supplied) parsers can take precedence.
	 *
	 * @param loader
	 *            service loader
