...
public class NetworkParser extends AbstractParser {
	...
	private void parse(TikaInputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
			throws IOException, SAXException, TikaException {
		if ("telnet".equals(uri.getScheme())) {
			...
			try {
				new ParsingTask(stream, connection.getOutputStream()).parse(new CloseShieldInputStream(input), handler,
						metadata, context);
			} finally {
				input.close();
			}
	...
	private static class ParsingTask implements Runnable {
		...
		public void parse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
			...
			try {
				context.getSAXParser().parse(stream, new TeeContentHandler(tagged, new MetaHandler(metadata)));
