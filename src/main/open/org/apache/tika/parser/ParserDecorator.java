...
/**
 * Decorator base class for the {@link Parser} interface. This class simply
 * delegates all parsing calls to an underlying decorated parser instance.
...
public class ParserDecorator extends AbstractParser {
	...
	/**
	 * Decorates the given parser so that it always claims to support parsing of
	...
	/**
	 * Decorates the given parser so that it never claims to support parsing of
	 * the given media types, but will work for all others.
	 *
	 * @param parser
	 *            the parser to be decorated
	 * @param types
	...
	public static final Parser withoutTypes(Parser parser, final Set<MediaType> excludeTypes) {
		return new ParserDecorator(parser) {
			...
			public Set<MediaType> getSupportedTypes(ParseContext context) {
				...
				Set<MediaType> parserTypes = new HashSet<MediaType>(super.getSupportedTypes(context));
	...
	/**
	 * Decorates the given parsers into a virtual parser, where they'll be tried
	 * in preference order until one works without error. TODO Is this the right
	 * name? TODO Is this the right place to put this? Should it be in
	 * CompositeParser? Elsewhere? TODO Should we reset the Metadata if we try
	 * another parser? TODO Should we reset the ContentHandler if we try another
	 * parser? TODO Should we log/report failures anywhere?
	 * 
	...
	public static final Parser withFallbacks(final Collection<? extends Parser> parsers, final Set<MediaType> types) {
		...
		return new ParserDecorator(parser) {
			...
			public void parse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
				...
				// Must have a TikaInputStream, so we can re-use it if parsing
				// fails
	 ...
	 * override this method (and use <code>super.getSupportedTypes()</code> to
	 * invoke the decorated parser) to implement extra decoration.
	 */
	public Set<MediaType> getSupportedTypes(ParseContext context) {
	...
	/**
	 * Delegates the method call to the decorated parser. Subclasses should
	 * override this method (and use <code>super.parse()</code> to invoke the
	 * decorated parser) to implement extra decoration.
	 */
	public void parse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
			throws IOException, SAXException, TikaException {
