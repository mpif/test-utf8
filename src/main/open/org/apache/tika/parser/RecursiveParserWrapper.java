...
/**
 * This is a helper class that wraps a parser in a recursive handler. It takes
 * care of setting the embedded parser in the ParseContext and handling the
 * embedded path calculations.
 * <p>
 * After parsing a document, call getMetadata() to retrieve a list of Metadata
 * objects, one for each embedded resource. The first item in the list will
 * contain the Metadata for the outer container file.
 * <p>
 * Content can also be extracted and stored in the {@link #TIKA_CONTENT} field
 * of a Metadata object. Select the type of content to be stored at
 * initialization.
 * <p>
 * If a WriteLimitReachedException is encountered, the wrapper will stop
 * processing the current resource, and it will not process any of the child
 * resources for the given resource. However, it will try to parse as much as it
 * can. If a WLRE is reached in the parent document, no child resources will be
 * parsed.
 * <p>
 * The implementation is based on Jukka's RecursiveMetadataParser and Nick's
 * additions. See: <a href=
 * "http://wiki.apache.org/tika/RecursiveMetadata#Jukka.27s_RecursiveMetadata_Parser">
 * RecursiveMetadataParser</a>.
 * <p>
 * Note that this wrapper holds all data in memory and is not appropriate for
 * files with content too large to be held in memory.
 * <p>
 * Note, too, that this wrapper is not thread safe because it stores state. The
 * client must initialize a new wrapper for each thread, and the client is
 * responsible for calling {@link #reset()} after each parse.
 * <p>
...
public class RecursiveParserWrapper implements Parser {
	...
	public final static Property PARSE_TIME_MILLIS = Property
			.internalText(TikaCoreProperties.TIKA_META_PREFIX + "parse_time_millis");
	public final static Property WRITE_LIMIT_REACHED = Property
			.internalBoolean(TikaCoreProperties.TIKA_META_EXCEPTION_PREFIX + "write_limit_reached");
	...
	public final static Property EMBEDDED_EXCEPTION = Property
			.internalText(TikaCoreProperties.TIKA_META_EXCEPTION_PREFIX + "embedded_exception");
	// move this to TikaCoreProperties?
	public final static Property EMBEDDED_RESOURCE_PATH = Property
			.internalText(TikaCoreProperties.TIKA_META_PREFIX + "embedded_resource_path");
	...
	/**
	 * Initialize the wrapper with {@link #catchEmbeddedExceptions} set to
	 * <code>true</code> as default.
	 *
	 * @param wrappedParser
	 *            parser to use for the container documents and the embedded
	 *            documents
	 * @param contentHandlerFactory
	 *            factory to use to generate a new content handler for the
	 *            container document and each embedded document
	 */
	 ...
	 *            factory to use to generate a new content handler for the
	 *            container document and each embedded document
	 * @param catchEmbeddedExceptions
	 *            whether or not to catch the embedded exceptions. If set to
	 *            <code>true</code>, the stack traces will be stored in the
	 *            metadata object with key: {@link #EMBEDDED_EXCEPTION}.
	 */
	public RecursiveParserWrapper(Parser wrappedParser, ContentHandlerFactory contentHandlerFactory,
	...
	/**
	 * Acts like a regular parser except it ignores the ContentHandler and it
	 * automatically sets/overwrites the embedded Parser in the ParseContext
	 * object.
	 * <p>
	...
	/**
	 * 
	 * The first element in the returned list represents the data from the outer
	 * container file. There is no guarantee about the ordering of the list
	 * after that.
	 * 
	...
	/**
	 * Set the maximum number of embedded resources to store. If the max is hit
	 * during parsing, the {@link #EMBEDDED_RESOURCE_LIMIT_REACHED} property
	 * will be added to the container document's Metadata.
	 * 
	 * <p>
	 * If this value is < 0 (the default), the wrapper will store all Metadata.
	 * 
	 * @param max
	 *            maximum number of embedded resources to store
	 */
	...
	/**
	 * Copied/modified from WriteOutContentHandler. Couldn't make that static,
	 * and we need to have something that will work with exceptions thrown from
	 * both BodyContentHandler and WriteOutContentHandler
	 * 
	 * @param t
	 * @return
	 */
	private boolean isWriteLimitReached(Throwable t) {
		if (t.getMessage() != null && t.getMessage().indexOf("Your document contained more than") == 0) {
			return true;
		} else {
	...
	private class EmbeddedParserDecorator extends ParserDecorator {
		...
		public void parse(InputStream stream, ContentHandler ignore, Metadata metadata, ParseContext context)
			...
			if (maxEmbeddedResources > -1 && metadatas.size() >= maxEmbeddedResources) {
