...
/**
 * Composite parser that delegates parsing tasks to a component parser based on
 * the declared content type of the incoming document. A fallback parser is
 * defined for cases where a parser for the given content type is not available.
 */
public class CompositeParser extends AbstractParser {
	...
	/**
	 * Utility method that goes through all the component parsers and finds all
	 * media types for which more than one parser declares support. This is
	 * useful in tracking down conflicting parser definitions.
	 *
	 * @since Apache Tika 0.10
	 * @see <a href="https://issues.apache.org/jira/browse/TIKA-660">TIKA-660
	 *      </a>
	 * @param context
	 *            parsing context
	...
	/**
	 * Returns all parsers registered with the Composite Parser, including ones
	 * which may not currently be active. This won't include the Fallback
	 * Parser, if defined
	 */
	 ...
	 * for a parser that matches the content type metadata property, and uses
	 * the fallback parser if a better match is not found. The type hierarchy
	 * information included in the configured media type registry is used when
	 * looking for a matching parser instance.
	 * <p>
	 * Subclasses can override this method to provide more accurate parser
	 * resolution.
	 *
	 * @param metadata
	 *            document metadata
