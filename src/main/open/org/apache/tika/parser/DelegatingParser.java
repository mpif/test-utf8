...
/**
 * Base class for parser implementations that want to delegate parts of the task
 * of parsing an input document to another parser. The delegate parser is looked
 * up from the parsing context using the {@link Parser} class as the key.
 *
...
public class DelegatingParser extends AbstractParser {
	...
	/**
	 * Looks up the delegate parser from the parsing context and delegates the
	 * parse operation to it. If a delegate parser is not found, then an empty
	 * XHTML document is returned.
	 * <p>
	 * Subclasses should override this method to parse the top level structure
	 * of the given document stream. Parsed sub-streams can be passed to this
	 * base class method to be parsed by the configured delegate parser.
	 */
