...
public abstract class ProxyInputStream extends FilterInputStream {
	...
	/**
	 * Invoked by the read methods before the call is proxied. The number of
	 * bytes that the caller wanted to read (1 for the {@link #read()} method,
	 * buffer length for {@link #read(byte[])}, etc.) is given as an argument.
	 * <p>
	 * Subclasses can override this method to add common pre-processing
	 * functionality without having to override all the read methods. The
	 * default implementation does nothing.
	 * <p>
	 * Note this method is <em>not</em> called from {@link #skip(long)} or
	 * {@link #reset()}. You need to explicitly override those methods if you
	 * want to add pre-processing steps also to them.
	 *
	 ...
	 * successfully. The number of bytes returned to the caller (or -1 if the
	 * end of stream was reached) is given as an argument.
	 * <p>
	 ...
	 * Note this method is <em>not</em> called from {@link #skip(long)} or
	 * {@link #reset()}. You need to explicitly override those methods if you
	 * want to add post-processing steps also to them.
	 *
	 * @since Commons IO 2.0
	 * @param n
