...
/**
 * Content type detection based on magic bytes, i.e. type-specific patterns near
 * the beginning of the document input stream.
 *
 * Because this works on bytes, not characters, by default any string matching
 * is done as ISO_8859_1. To use an explicit different encoding, supply a type
 * other than "string" / "stringignorecase"
 *
...
public class MagicDetector implements Detector {
	...
	/**
	 * The magic match pattern. If this byte pattern is equal to the possibly
	 * bit-masked bytes from the input stream, then the type detection succeeds
	 * and the configured {@link #type} is returned.
	 */
	private final byte[] pattern;

	/**
	 * Length of the pattern, which in the case of regular expressions will not
	 * be the same as the comparison window length.
	 */
	...
	/**
	 * First offset (inclusive) of the comparison window within the document
	 * input stream. Greater than or equal to zero.
	 */
	private final int offsetRangeBegin;

	/**
	 * Last offset (inclusive) of the comparison window within the document
	 * input stream. Greater than or equal to the {@link #offsetRangeBegin first
	 * offset}.
	 * <p>
	 * Note that this is <em>not</em> the offset of the last byte read from the
	 * document stream. Instead, the last window of bytes to be compared starts
	 * at this offset.
	 */
	 ...
	 * match. {@code pattern} must NOT be a regular expression. Constructor
	...
	public MediaType detect(InputStream input, Metadata metadata) throws IOException {
		...
		try {
			...
			while (n != -1 && offset < offsetRangeEnd + length) {
				...
				// increment offset - in case not all read (see
				// testDetectStreamReadProblems)
				if (n > 0) {
					offset += n;
				}
			}
	...
	/**
	 * Returns a string representation of the Detection Rule. Should sort nicely
	 * by type and details, as we sometimes compare these.
	 */
	public String toString() {
		// Needs to be unique, as these get compared.
		return "Magic Detection for " + type + " looking for " + pattern.length + " bytes = " + this.pattern
				+ " mask = " + this.mask;
	}
}
