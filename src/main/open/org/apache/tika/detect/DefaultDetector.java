 ...
 * followed by non-MimeType Tika, followed by the Tika MimeType class. If you
 * need to control the order of the Detectors, you should instead construct your
 * own {@link CompositeDetector} and pass in the list of Detectors in the
 * required order.
 *
...
public class DefaultDetector extends CompositeDetector {
	...
	/**
	 * Finds all statically loadable detectors and sort the list by name, rather
	 * than discovery order. Detectors are used in the given order, so put the
	 * Tika parsers last so that non-Tika (user supplied) parsers can take
	 * precedence.
	 *
	 * @param loader
	 *            service loader
