...
/**
 * Contains a core set of basic Tika metadata properties, which all parsers will
 * attempt to supply (where the file format permits). These are all defined in
 * terms of other standard namespaces.
 * 
 * Users of Tika who wish to have consistent metadata across file formats can
 * make use of these Properties, knowing that where present they will have
 * consistent semantic meaning between different file formats. (No matter if one
 * file format calls it Title, another Long-Title and another Long-Name, if they
 * all mean the same thing as defined by {@link DublinCore#TITLE} then they will
 * all be present as such)
 *
 * For now, most of these properties are composite ones including the deprecated
 * non-prefixed String properties from the Metadata class. In Tika 2.0, most of
 * these will revert back to simple assignments.
 * 
...
public interface TikaCoreProperties {
	...
	/**
	 * A file might contain different types of embedded documents. The most
	 * common is the ATTACHEMENT. An INLINE embedded resource should be used for
	 * embedded image files that are used to render the page image (as in
	 * PDXObjImages in PDF files).
	 * <p>
	...
	/**
	 * Use this to prefix metadata properties that store information about the
	 * parsing process. Users should be able to distinguish between metadata
	 * that was contained within the document and metadata about the parsing
	 * process. In Tika 2.0 (or earlier?), let's change X-ParsedBy to
	 * X-TIKA-Parsed-By.
	 */
	...
	/**
	 * This is currently used to identify Content-Type that may be included
	 * within a document, such as in html documents (e.g.
	 * <meta http-equiv="content-type" content="text/html; charset=UTF-8">) , or
	 * the value might come from outside the document. This information may be
	 * faulty and should be treated only as a hint.
	 */
	public static final Property CONTENT_TYPE_HINT = Property.internalText(HttpHeaders.CONTENT_TYPE + "-Hint");
	 ...
	 */
	public static final Property CREATOR = Property.composite(DublinCore.CREATOR, new Property[] { Office.AUTHOR,
			Property.internalTextBag(Metadata.CREATOR), Property.internalTextBag(Metadata.AUTHOR) });
	 ...
	 * @see Office#KEYWORDS
	 */
	public static final Property KEYWORDS = Property.composite(DublinCore.SUBJECT, new Property[] { Office.KEYWORDS,
			Property.internalTextBag(MSOffice.KEYWORDS), Property.internalTextBag(Metadata.SUBJECT) });
	...
	public static final Property CREATED = Property.composite(DublinCore.CREATED,
			new Property[] { Office.CREATION_DATE, MSOffice.CREATION_DATE });
	...
	public static final Property MODIFIED = Property.composite(DublinCore.MODIFIED,
			new Property[] { Metadata.DATE, Office.SAVE_DATE, MSOffice.LAST_SAVED,
					Property.internalText(Metadata.MODIFIED), Property.internalText("Last-Modified") });
	 ...
	 */
	public static final Property COMMENTS = Property.composite(OfficeOpenXMLExtended.COMMENTS, new Property[] {
			Property.internalTextBag(ClimateForcast.COMMENT), Property.internalTextBag(MSOffice.COMMENTS) });
	 ...
	 */
	public static final Property EMBEDDED_RESOURCE_TYPE = Property.internalClosedChoise(
			TikaMetadataKeys.EMBEDDED_RESOURCE_TYPE,
