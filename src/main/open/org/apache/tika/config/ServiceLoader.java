...
public class ServiceLoader {
	...
	/**
	 * The dynamic set of services available in an OSGi environment. Managed by
	 * the {@link TikaActivator} class and used as an additional source of
	 * service instances in the {@link #loadServiceProviders(Class)} method.
	 */
	private static final Map<Object, RankedService> services = new HashMap<Object, RankedService>();

	/**
	 * Returns the context class loader of the current thread. If such a class
	 * loader is not available, then the loader of this class or finally the
	 * system class loader is returned.
	 *
	 * @see <a href="https://issues.apache.org/jira/browse/TIKA-441">TIKA-441
	 *      </a>
	 * @return context class loader, or <code>null</code> if no loader is
	 *         available
	 */
	...
	/**
	 * Sets the context class loader to use for all threads that access this
	 * class. Used for example in an OSGi environment to avoid problems with the
	 * default context class loader.
	 *
	 * @param loader
	 *            default context class loader, or <code>null</code> to
	 *            automatically pick the loader
	 */
	...
	public ServiceLoader() {
		this(getContextClassLoader(), Boolean.getBoolean("org.apache.tika.service.error.warn") ? LoadErrorHandler.WARN
	 ...
	 *             if the service class can not be found or does not implement
	 *             the given interface
	...
	/**
	 * Returns all the available service resources matching the given pattern,
	 * such as all instances of tika-mimetypes.xml on the classpath, or all
	...
	/**
	 * Returns the available dynamic service providers of the given type. The
	 * returned list is newly allocated and may be freely modified by the
	 * caller.
	 *
	 ...
	 * attempting to load them. The providers are loaded using the service
	 * provider mechanism using the configured class loader (if any).
	 *
	...
	/**
	 * Returns the available static service providers of the given type. The
	 * providers are loaded using the service provider mechanism using the
	 * configured class loader (if any). The returned list is newly allocated
	...
	private void collectServiceClassNames(URL resource, Collection<String> names) throws IOException {
		...
		try {
			BufferedReader reader = new BufferedReader(new InputStreamReader(stream, IOUtils.UTF_8));
