...
class ClassLoaderResource implements ForkResource {
	 ...
	 * <code>true</code> value. If the resource was not found (code 1) or when
	 * the last resource has been sent (code 2), a boolean <code>false</code>
	...
	/**
	 * Sends the contents of the given input stream to the given output. The
	 * stream is sent in chunks of less than 64kB, each preceded by a 16-bit
	 * integer value that indicates the length of the following chunk. A zero
	 * short value is sent at the end to signify the end of the stream.
	 * <p>
	 * The stream is guaranteed to be closed by this method, regardless of the
	 * way it returns.
	 *
	 * @param stream
