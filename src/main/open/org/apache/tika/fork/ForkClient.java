...
class ForkClient {
	...
	public ForkClient(ClassLoader loader, Object object, List<String> java) throws IOException, TikaException {
		boolean ok = false;
		try {
			this.loader = loader;
	...
	/**
	 * Serializes the object first into an in-memory buffer and then writes it
	 * to the output stream with a preceding size integer.
	 *
	 * @param object
	...
	/**
	 * Consumes all pending bytes from the standard error stream of the forked
	 * server process, and prints them out to the standard error stream of this
	 * process. This method should be called always before expecting some output
	 * from the server, to prevent the server from blocking due to a filled up
	 * pipe buffer of the error stream.
	 *
	 * @throws IOException
	 *             if the error stream could not be read
	 */
