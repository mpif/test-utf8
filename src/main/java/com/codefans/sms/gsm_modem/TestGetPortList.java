package com.codefans.sms.gsm_modem;
...
public class TestGetPortList {
	...
	public static void getCommPortList() {
		...
		while (portEnum.hasMoreElements()) {
			...
			if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL) {
				System.out.println("串口: name-" + portId.getName() + " 是否占用-" + portId.isCurrentlyOwned());
			} else {
				System.out.println("并口: name-" + portId.getName() + " 是否占用-" + portId.isCurrentlyOwned());
			}
		}
		System.out.println("-------------------------------------");
	}
	...
	public static void getPortByDriver() {
		...
		try {
			...
			sPort = (SerialPort) commDriver.getCommPort("COM24", CommPortIdentifier.PORT_SERIAL);
