/*
 * @(#)UUDecoderStream.java	1.4 00/07/14
 *
...
public class UUDecoderStream extends FilterInputStream {
	...
	private boolean decode() throws IOException {
		...
		/*
		 * Improperly encoded data sometimes omits the zero length line that
		 * starts with a space character, we detect the following "end" line
		 * here.
		 */
		if (line == null)
		...
		/*
		 * The first character in a line is the number of original (not the
		 * encoded atoms) characters in the line. Note that all the code below
		 * has to handle the <SPACE> character that indicates end of encoded
		 * stream.
		 */
		count = (count - ' ') & 0x3f;
		...
		// System.out.println("count " + count + ", need " + need + ", len " +
		// line.length());
	 ...
	 * UUDecoderStream decoder = new UUDecoderStream(infile); int c;
	 * 
	 * try { while ((c = decoder.read()) != -1) System.out.write(c);
	 * System.out.flush(); } catch (Exception e) { e.printStackTrace(); } } end
	 * TEST program
	 ****/
}
