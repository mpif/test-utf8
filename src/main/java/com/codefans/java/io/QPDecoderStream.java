/*
 * @(#)QPDecoderStream.java	1.7 00/03/16
 *
...
/**
 * This class implements a QP Decoder. It is implemented as a FilterInputStream,
...
public class QPDecoderStream extends FilterInputStream {
	...
	/**
	 * Create a Quoted Printable decoder that decodes the specified input
	 * stream.
	 * 
	 * @param in
	 *            the input stream
	...
	public int read() throws IOException {
		...
		if (c == ' ') {
			...
			if (a == '\n') {
				/*
				 * Hmm ... not really confirming QP encoding, but lets allow
				 * this as a LF terminated encoded line .. and consider this a
				 * soft linebreak and recurse to fetch the next char.
				 */
				return read();
			} else if (a == '\r') {
				...
				if (b != '\n')
					/*
					 * Not really confirming QP encoding, but lets allow this as
					 * well.
					 */
	 ...
	 * without blocking. The QP algorithm does not permit a priori knowledge of
	 * the number of bytes after decoding, so this method just invokes the
	 * <code>available</code> method of the original input stream.
	 */
	 ...
	 * QPDecoderStream decoder = new QPDecoderStream(infile); int c;
	 * 
