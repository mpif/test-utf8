/*
 * @(#)QPEncoderStream.java	1.3 99/12/06
 *
...
/**
 * This class implements a Quoted Printable Encoder. It is implemented as a
...
public class QPEncoderStream extends FilterOutputStream {
	 ...
	 *            the output stream
	 * @param bytesPerLine
	 *            the number of bytes per line. The encoder inserts a CRLF
	 *            sequence after this many number of bytes.
	 */
	...
	/**
	 * Create a QP encoder that encodes the specified input stream. Inserts the
	 * CRLF sequence after outputting 76 bytes.
	 * 
	 * @param out
	 *            the output stream
	 */
	 ...
	 */
	/*
	 * public void write(int c) throws IOException { c = c & 0xff; // Turn off
	 * the MSB. if (gotSpace) { // previous character was <SPACE> if (c == '\r'
	 * || c == '\n') // if CR/LF, we need to encode the <SPACE> char output(' ',
	 * true); else // no encoding required, just output the char output(' ',
	 * false); gotSpace = false; }
	 * 
	 * if (c == ' ') { gotSpace = true; } else if (c == '\r') { gotCR = true;
	 * outputCRLF(); } else if (c == '\n') { if (gotCR) // This is a CRLF
	 * sequence, we already output the // corresponding CRLF when we got the CR,
	 * so ignore this gotCR = false; else outputCRLF(); } else if (c < 040 || c
	 * >= 0177 || c == '=') // Encoding required. output(c, true); else // No
	 * encoding required output(c, false); }
	 */
	...
	private final void outputCRLF() throws IOException {
		out.write('\r');
		out.write('\n');
		count = 0;
	}
	...
	protected final void output(int c, boolean encode) throws IOException {
		if (encode) {
	 ...
	 * QPEncoderStream encoder = new QPEncoderStream(System.out); int c;
	 * 
	...
	public static void main(String[] argv) throws Exception {
		QPEncoderStream out = new QPEncoderStream(System.out);
		out.write("test \rdslfkd\r\ndsfkdsf\nskdlfldks dkfd\r\n".getBytes());
		out.flush();
