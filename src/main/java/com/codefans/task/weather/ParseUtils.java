package com.codefans.task.weather;
...
public class ParseUtils {
	...
	private static String getContent(HttpEntity entity, String encode) throws Exception {
		if (entity != null) {
	...
	public static <T extends Tag> void getInnerHTML(String url, Class<T> clazz) throws ParserException {
	...
	public static <T extends Tag> void getTagAttribute(String url, Class<T> clazz, String attr) throws ParserException {
		Parser parser = new Parser(url);
		NodeFilter filter = new NodeClassFilter(clazz);
		NodeList list = parser.extractAllNodesThatMatch(filter);
		for (int i = 0; i < list.size(); i++) {
	...
	public static String getElementByAttr(String url, String attr, String value) throws ParserException {
		Parser p = new Parser(url);
	...
	/**
	 * 使用HtmlParser抓去网页内容: 要抓去页面的内容便的方法就是使用StringBean. 里面有几个控制页面内容的几个参数.
	 * 在后面的代码中会有说 Htmlparser包中还有示例StringExtractor 里面有个直接得到内容的方 其中也是使用了StringBean
	 * . 另外直接解析Parser的每个标签也可以
	 */
